<div class="space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row">
            <div class="flex-1">
                <flux:input wire:model.live.debounce.300ms="search" placeholder="{{ __('Search devices...') }}" icon="magnifying-glass" />
            </div>
            <flux:select wire:model.live="filterType" placeholder="{{ __('All Types') }}">
                <flux:select.option value="">{{ __('All Types') }}</flux:select.option>
                @foreach (\App\Enums\DeviceType::cases() as $type)
        ...
        <flux:table :paginate="$this->devices">
            <flux:table.columns>
                ...
                <flux:table.column sortable :sorted="$sortBy === 'last_activity_at'" :direction="$sortDirection" wire:click="sort('last_activity_at')">{{ __('Last Activity') }}</flux:table.column>
            ...
            <flux:table.rows>
                @forelse ($this->devices as $device)
                    <flux:table.row :key="$device->id">
                        ...
                        <flux:table.cell>
                            <flux:badge size="sm" :color="$device->status->color()" inset="top bottom">{{ $device->status->label() }}</flux:badge>
                        </flux:table.cell>
                        <flux:table.cell class="font-mono text-xs">{{ Str::limit($device->device_id, 18) }}</flux:table.cell>
                        <flux:table.cell class="whitespace-nowrap">{{ $device->last_activity_at?->diffForHumans() ?? '—' }}</flux:table.cell>
                        <flux:table.cell class="whitespace-nowrap">{{ $device->created_at->diffForHumans() }}</flux:table.cell>
                        <flux:table.cell>
                            <flux:dropdown position="bottom" align="end">
                                <flux:button variant="ghost" size="sm" icon="ellipsis-horizontal" inset="top bottom" />
                                <flux:menu>
                                    <flux:menu.item icon="eye" :href="route('devices.show', $device)" wire:navigate>{{ __('View') }}</flux:menu.item>
                                    <flux:menu.item icon="pencil-square" :href="route('devices.edit', $device)" wire:navigate>{{ __('Edit') }}</flux:menu.item>
                                    <flux:menu.separator />
                                    <flux:menu.item icon="trash" variant="danger" wire:click="deleteDevice({{ $device->id }})" wire:confirm="{{ __('Are you sure you want to delete this device?') }}">{{ __('Delete') }}</flux:menu.item>
                                </flux:menu>
                            </flux:dropdown>
                        </flux:table.cell>
                    </flux:table.row>
                    ...
                    <flux:table.row>
                        <flux:table.cell colspan="7" class="text-center">
                            <div class="py-8">
                                ...
                                <flux:button variant="primary" size="sm" icon="plus" :href="route('devices.create')" wire:navigate class="mt-4">
                                    {{ __('Add Device') }}
                                </flux:button>
                            </div>
                        </flux:table.cell>
