<div class="mx-auto max-w-2xl space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <flux:heading size="xl">{{ __('Create Cloud Variable') }}</flux:heading>
            <flux:text class="mt-1">{{ $thing->name }}</flux:text>
        </div>
    </div>

    <flux:card>
        <form wire:submit="createVariable" class="space-y-4">
            <flux:input wire:model.live.debounce.300ms="name" :label="__('Name')" placeholder="{{ __('e.g. Room Temperature') }}" required autofocus autocomplete="off" />
            <flux:error name="name" />

            @if ($this->variableName)
                <div class="rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800">
                    <flux:text class="text-xs font-medium">{{ __('Variable Name') }}</flux:text>
                    <code class="mt-1 block text-sm">{{ $this->variableName }}</code>

                    @if ($this->declaration)
                        <flux:text class="mt-2 text-xs font-medium">{{ __('C++ Declaration') }}</flux:text>
                        <code class="mt-1 block text-sm">{{ $this->declaration }}</code>
                    @endif
                </div>
            @endif
            ...
            <div class="flex items-center gap-3">
                <flux:button variant="primary" type="submit">{{ __('Create Variable') }}</flux:button>
