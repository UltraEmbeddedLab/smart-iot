<div class="mx-auto max-w-2xl space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <flux:heading size="xl">{{ __('Create Trigger') }}</flux:heading>
            <flux:text class="mt-1">{{ __('Set up an automation that fires when a variable meets a condition.') }}</flux:text>
        </div>
    </div>

    <flux:card>
        <form wire:submit="createTrigger" class="space-y-4">
            <flux:input wire:model="name" :label="__('Name')" placeholder="{{ __('e.g. High Temperature Alert') }}" required autofocus autocomplete="off" />
            ...
            <div class="flex items-center gap-3">
                <flux:button variant="primary" type="submit">{{ __('Create Trigger') }}</flux:button>
